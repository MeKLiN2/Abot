...
class TinychatBot(pinylib.TinychatRTCClient):
    ...
    def on_join(self, join_info):
        ...
        #lockdowncheck = self.spamcheck(_user, time_join)
        ...
        if pinylib.CONFIG.B_ALLOW_BROADCASTS:
            if _user is not None:
                if _user.user_level == 8 or _user.nick in self.cambans:
                    ...
                    self.handle_msg('%s is being abused by mods' % _user.nick)
    ...
    def on_pending_moderation(self, pending):
        ...
        if _user is not None:
            ...
            if _user.user_level < 5:
                ...
                self.send_chat_msg('%s will probably never get on cam' % _user.nick)
    ...
    def cmd_handler(self, msg):
        ...
        elif cmd == prefix + '8ball7':
            self.do_8ball(cmd_arg)

        elif cmd == prefix + 'roll7':
            self.do_dice()

        elif cmd == prefix + 'flip7':
            self.do_flip_coin()
        ...
        if cmd == prefix + 'holden':
            ...
            if _user is not None:
                ...
                self.handle_msg('Nickname?')
            self.handle_msg('%s is being abused by mods' % cmd_arg)
    ...
    def do_help(self):
        ...
        ownr_cmds = ["reboot", "kb", "p2t", "dir", "holden"]
        admin_cmds = ["+mod", "-mod"]
        mod_cmds = ["announcement", "lockdown", "lockup", "+tmod", "-tmod", "spam", "vip", "verbose", "acc",
                    "allowcam"]
        cmod_cmds = ["clr", "kick", "ban", "unb", "sbl", "fg", "cam", "close", "+banwword", "-badword",
                     "noguest",
        ...
        if self.active_user.user_level >= 4 and self.active_user.user_level > 5:
